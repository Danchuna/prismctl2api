...
// 保存到 bootstrap.json：不落盘的话，/v1 调用仍会用启动时载入的旧 token
async function saveBootstrap() {
  try {
    var f = form();
    var res = await call('/api/bootstrap', {
      cookie: f.cookie,
      sandbox_token: f.sandboxToken,
      sandbox_url: f.sandboxUrl,
      project_id: f.projectId,
      user_id: f.userId,
      conversation_id: f.conversationId,
      listen_snapshot: f.listenSnapshot
    });
    var j = {};
    try { j = JSON.parse(res.text); } catch (e) {}
    var msg = j.ok
      ? '已保存 → ' + (j.saved_to || '(未配置 -bootstrap 路径)') + ' ｜ token ' + j.sandbox_token_len +
        'B · snapshot ' + j.listen_snapshot_len + 'B · cookie ' + j.cookie_len + 'B'
      : '保存失败：' + (j.error ? j.error.message : res.text.slice(0, 200));
    $('saveMsg').textContent = msg;
    appendLog('[save] ' + msg);
  } catch (e) {
    $('saveMsg').textContent = '保存失败：' + e;
    appendLog('!! save 失败: ' + e);
  }
}
